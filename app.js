const jobs = [
  ...
  },
  {
    ...
    description:
      "Build enterprise applications with modern JavaScript frameworks.",
    status: "All",
  },
  {
    id: 7,
    ...
    description: "Join a fast-growing startup and work on core platform.",
    status: "All",
  },
  {
    id: 8,
...
function renderJobs() {
  ...
  document.getElementById("jobsNumber").innerText = filtered.length + " jobs";
...
function updateDashboard() {
  ...
  document.getElementById("countInterview").innerText = jobs.filter(
    (j) => j.status === "Interview",
  ).length;
  document.getElementById("countRejected").innerText = jobs.filter(
    (j) => j.status === "Rejected",
  ).length;
}
